...
def get_streaming_response(question, session_id, model):
    headers = {
...
def upload_document(file):
    ...
    try:
        ...
        response = requests.post("http://localhost:8083/upload-doc", files=files)
        if response.status_code == 200:
...
def list_documents():
    try:
        response = requests.get("http://localhost:8083/list-docs")
        if response.status_code == 200:
            return response.json()
        else:
...
def delete_document(file_id):
    ...
    try:
        response = requests.post("http://localhost:8083/delete-doc", headers=headers, json=data)
        if response.status_code == 200:
            return response.json()
        else:
